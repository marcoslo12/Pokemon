		...
		for (let i = 0; i < 10; i++) {
			...
			fetch('https://pokeapi.co/api/v2/pokemon/' + sessionStorage.getItem('Pokemon').split(',')[i])
				...
				.then((data) => {
					...
					//veure mes
					newItem +=
						'<button onclick="mesinfo(' +
						sessionStorage.getItem('Pokemon').split(',')[i] +
...
function buscar() {
	...
	// Existe una función que te dice true si un texto empieza por la cadena de texto buscada
	// startsWith => https://developer.mozilla.org/es/docs/Web/JavaScript/Reference/Global_Objects/String/startsWith
	// if (!pokemonsearch[i].startsWith(input)) {
